...
for i = 1:1001
  %changed m to be opposite of what I think it should be
   ...
   gc3d_abs_LX(1,i) = 1/(2*pi^2) * (2*mL/hbar^2)^(3/2) * real(sqrt(E(i) + E_LX - delta_E_XL));
   gc3d_em_LX(1,i) = 1/(2*pi^2) * (2*mL/hbar^2)^(3/2) * real(sqrt(E(i) - E_LX - delta_E_XL));

   gc3d_abs_XL(1,i) = 1/(2*pi^2) * (2*mX/hbar^2)^(3/2) * real(sqrt(E(i) - E_LX + delta_E_XL));
   gc3d_em_XL(1,i) = 1/(2*pi^2) * (2*mX/hbar^2)^(3/2) * real(sqrt(E(i) + E_LX + delta_E_XL));

   gc3d_abs_XX(1,i) = 1/(2*pi^2) * (2*mL/hbar^2)^(3/2) * real(sqrt(E(i) + E_XX));
   gc3d_em_XX(1,i) = 1/(2*pi^2) * (2*mL/hbar^2)^(3/2) * real(sqrt(E(i) - E_XX));
...
% momentum relaxation rates for every given energy, then invert the sum to get
% the total momentum relaxation rate, and then find the average of that
% time, <<?m>>. With it, calculate the mobility and conductivity. How does it
% compare to that obtained using Matthiessen?s rule? How does it compare to
% experiment/your calculation using Rode?s method?
function out = scatteringMechs()
